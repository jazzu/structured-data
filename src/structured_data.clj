...
(defn author->string [author]
  (let [name (:name author)
        ...
        by-exists (not (or (= "" birth-year) (nil? birth-year)))]
    (str name
         (if by-exists
           (str " ("
                birth-year
...
(defn authors->string [authors]
  (let [commafy (fn [author]
                  (str (author->string author) ", "))]
    (apply str (concat (map commafy (butlast authors))
                       (author->string (last authors))))))
...
(defn books->string [books]
  (let [c (count books)
    ...
    (str count-str
         (apply str
                (concat (map stopfy (butlast books))
                        (book->string (last books))))
         ".")))
